...
public interface UserRepository extends JpaRepository<User,Long> {
    ...
    @Query("SELECT u FROM User u WHERE u.subDistrict.district.id = :districtId  AND 'USER' MEMBER OF u.roles")
    List<User> findByDistrictId(@Param("districtId") Long districtId);
    @Query("SELECT u FROM User u WHERE u.subDistrict.id = :subDistrictId   AND 'USER' MEMBER OF u.roles")
    List<User> findBySubDistrictId(@Param("subDistrictId") Long subDistrictId);
    @Query("SELECT u FROM User u WHERE u.subDistrict.district.id = :districtId AND u.subDistrict.id = :subDistrictId  AND 'USER' MEMBER OF u.roles ")
